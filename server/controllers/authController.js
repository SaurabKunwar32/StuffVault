  ...
  if (user) {
    if (user.isDeleted) {
      return res.status(403).json({
        error:
          "Your account has been deleted for recover please contact to Admin ",
      });
    }
...
export const loginWithGithub = async (req, res) => {
  ...
  if (referer) {
    try {
      ...
      clientURL = url.origin; // strips trailing slash safely
    } catch {
...
export const callbackGithub = async (req, res, next) => {
  ...
  if (!success) {
    return res.status(400).json({
      error: "GitHub OAuth failed: Authorization code is missing or invalid.",
    });
  }
  ...
  try {
    ...
    /* ---- New user creation (unchanged logic) ---- */
